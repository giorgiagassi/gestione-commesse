...
@Component({
  selector: 'app-tipologia-appalto',
  standalone: true,
    imports: [
        Button,
        PrimeTemplate,
  ...
  templateUrl: './tipologia-appalto.component.html',
  styleUrl: './tipologia-appalto.component.css'
})
export class TipologiaAppaltoComponent implements OnInit{
  ...
  async getCommesseList() {
    const huntersRef = ref(database, 'impostazioni/tipologia-appalto');
    try {
  ...
  editCustomer(customer: any) {
    ...
    this.router.navigate(['/modifica-appalto', customer.id]);
  }
  deleteCustomer(customer: any) {
    // Assicurati di avere un DatabaseReference corretto
    const customerRef = ref(database, `impostazioni/tipologia-appalto/${customer.id}`);
