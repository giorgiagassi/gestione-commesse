...
@Component({
  ...
  templateUrl: './spese-postali.component.html',
  styleUrl: './spese-postali.component.css'
})
export class SpesePostaliComponent implements OnInit{
  ...
  async getCommesseList() {
    const huntersRef = ref(database, 'impostazioni/spese-postali');
    try {
  ...
  editCustomer(customer: any) {
    ...
    this.router.navigate(['/modifica-spese-postali', customer.id]);
  }
  deleteCustomer(customer: any) {
    // Assicurati di avere un DatabaseReference corretto
    const customerRef = ref(database, `impostazioni/spese-postali/${customer.id}`);

    remove(customerRef).then(() => {
      // Rimuovi l'elemento dall'array per aggiornare l'UI
      this.spesePostaliList = this.spesePostaliList.filter((item: any) => item.id !== customer.id);
    }).catch((error:any) => {
    });
  }
}
